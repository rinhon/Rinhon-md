...
lang: zh-CN
---
这是一个非常专业且具体的自动化需求。使用 PowerShell 结合 ImageMagick 是处理此类任务的最佳选择，因为 ImageMagick 的命令行处理图片能力极强，而 PowerShell 能很好地与 Windows 系统交互。

### 前置准备

1. **安装 ImageMagick**: 脚本依赖 `magick` 命令。
    - 请前往 [ImageMagick官网](https://www.google.com/search?q=https://imagemagick.org/script/download.php%23windows) 下载 Windows 版本（建议下载 `ImageMagick-x.x.x-Q16-HDRI-x64-dll.exe` [点击下载](https://imagemagick.org/archive/binaries/ImageMagick-7.1.2-11-Q16-HDRI-x64-dll.exe "版本 ImageMagick-7.1.2-11-Q16-HDRI-x64-dll.exe")。
...
请在任意位置（建议固定目录，如 `C:\Scripts\FolderCover`）创建一个名为 `Set-FolderCover.ps1` 的文件，并粘贴以下代码。
这个脚本实现了：
1. 自动判断输入是文件夹还是图片。
...
if (-not (Test-Path $TargetScript)) {
    Write-Host "错误：找不到 Set-FolderCover.ps1，请确保两个脚本在同一目录下。" -ForegroundColor Red
    Pause
    Exit
}
...
try {
    ...
    Write-Host "警告：未检测到 'magick' 命令。请确保安装了 ImageMagick 并添加到了系统环境变量 PATH 中。" -ForegroundColor Yellow
...
4. **查看结果**：
    - 如果封面没有立即更新，请按 `F5` 刷新，或等待几秒钟（Windows 缩略图缓存可能有延迟）。
5. **排查错误**：
    - 如果没反应，请打开 `文档` 文件夹，查看 `FolderCover_ErrorLog.txt`。
